...
Create `evidence/article_evidence.csv` and `evidence/article_evidence.md`.

Every manuscript number must map to a source ID, raw value, transformation, processed dataset and exact output table/figure.

Block article generation if a material claim has status `to_replicate`, `unresolved` or lacks provenance.
