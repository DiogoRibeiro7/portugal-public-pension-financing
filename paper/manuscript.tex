...
\paragraph{[Unresolved evidence]}
The employer question is bounded in the same way, and the legal benchmark against which it would be tested is itself only partly established. Statutory worker and employer contribution-rate intervals are reconstructed by employer class from 2006 onward; the pre-2006 intervals, and direct extraction from the budget-law articles that set them, remain incomplete. The repository holds the distinction between statutory contribution rates and economic benchmark rates, which is the conceptual prerequisite for the test, but quantifying an employer shortfall additionally requires payroll bases and recorded employer-revenue splits. The evidentiary test can therefore be specified; a number cannot be reported.
